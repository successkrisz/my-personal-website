...
describe('(Component) GitHubWidget', () => {
  ...
  it('Should render link to GitHub user page', () => {
    _wrapper = render(<GitHubWidget username='successkrisz' repos={repos} fetching={false} />);
  ...
  it('Should render link for each repo', () => {
    _wrapper = shallow(<GitHubWidget username='successkrisz' repos={repos} fetching={false} />);
  ...
  it('Should render a spinner while fetching', () => {
    _wrapper = shallow(<GitHubWidget username='successkrisz' repos={null} fetching />);
    const link = _wrapper.find('li').find('a');
